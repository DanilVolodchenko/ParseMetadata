...
from third_party.parametrica import (
    Fieldset,
    Field,
    InRange,
    Min,
...
from third_party.parametrica.io import YAMLFileConfigIO, VirtualYAMLFileConfigIO
from third_party.parametrica.predefined.network import (
    HTTPServer,
    Credentials,
    PortField,
)
...
class Common(Fieldset):
    ...
    flag = (
        Field[bool](False)
        .label("Флаг значение")
        .hint("Тут понятно должно быть что оно в себе хранит")
    )
    string = (
        Field[str]("STR")
...
class MoreFields(Fieldset):
    ...
    email = (
        Field[str]("some_email@gmail.com")
        .label("Email адрес")
        .rule(Match(r"([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)"))
    )
    secret_token = (
        Field[str](uuid4)
        .label("Супер секретный токен")
        .hint("Его нигде и ниеогда не палить")
        .secret()
    )
...
class Config(Parametrica):
    common = (
        Field[Common]().label("Общие поля для примера").hint("Сюда вписывать примитивы")
    )
    ...
    custom_default = Field[str]("CustomDefault").label(
        "Кастомное дефолт значение 1, которое никогда не появится"
    )
    custom_default = Field[CustomDefaultVal](CustomDefaultVal(default_1234=1111)).label(
        "Кастомное дефолт значение 1"
    )
    custom_default_2 = Field[CustomDefaultVal](default_1234=2222).label(
        "Кастомное дефолт значение 2"
    )
