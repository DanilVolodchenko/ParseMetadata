...
class YAMLFileConfigIO(FileConfigIOInterface):
    ...
    def make_comments(self, fieldset: '_FieldRW', dataset: dict, resultstr: str, parent_fieldset: str = '',
                      indent: int = 0) -> str:
        for field_name in dataset.keys():
            ...
            if parent_fieldset == '':
                ...
                search = re.compile(
                    r'\A.*{}{}\:.*?{}({})\:'.format((indent - 2) * " ", parent_fieldset, indent * " ", field_name),
                    re.RegexFlag.S)
            ...
            if not field.__is_iterable_type__() and not field.__is_primitive_type__():
                resultstr = self.make_comments(field.__get__(fieldset, fieldset.__class__), dataset.get(field_name),
                                               resultstr, field_name, indent + 2)

        return resultstr

    def serialize(self, dataset: dict) -> str:
        resultstr = self.yaml.dump(dataset, sort_keys=False, default_flow_style=False, default_style=None,
                                   allow_unicode=True, canonical=None, Dumper=self.yaml.SafeDumper)
